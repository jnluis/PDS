...
public class PDS2122 {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		String[] logFiles = new File(suser).list((d, s) -> {
			return s.equals("src");
		});
		String sp = "";
	...
	private static void question2(PrintStream out) {
		...
		market.loadProducts(new VectorProductDataSource(cars));
		market.loadProducts(new TextFileProductDataSource("products.txt"));
	...
	private static void question3(PrintStream out) {
		...
		u1.setPrintStream(out);
		u2.setPrintStream(out);
		u3.setPrintStream(out);

		market.share("ZA11ZB", u1);
		market.share(cars[2].code(), u2);
		market.share("BB44ZB", u3);

		market.share("ZA11ZB", u2); // Anne interessada no Tesla
		market.share("BB44ZB", u1); // Peter interessado no Ford Mustang

		out.println("--- Shared Products ---\n" + market.sharedProducts());

		market.giveBack(cars[0].code());
		market.giveBack("BB44ZB");

		out.println("--- Shared Products ---\n" + market.sharedProducts());

		// Anne e Peter adicionam um produtos que queriam
		market.share("BB44ZB", u1);
		market.share("ZA11ZB", u2);
